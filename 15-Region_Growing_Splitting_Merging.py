...
def region_merging(image, threshold):
    ...
    def merge_regions(x, y, current_label):
        ...
        while queue:
            ...
            cx, cy = current_pixel
            if (0 <= cx < width and 0 <= cy < height and labels[cy, cx] == 0 and abs(
                    int(image[cy, cx]) - int(image[y, x])) < threshold):
...
cv2.imshow("Segmented Region", region_growing(image, threshold, (130, 110)))
cv2.imshow("Region Spliting", region_splitting(image, threshold))
cv2.imshow("Region Merging", region_merging(image, threshold))
cv2.waitKey(0)
